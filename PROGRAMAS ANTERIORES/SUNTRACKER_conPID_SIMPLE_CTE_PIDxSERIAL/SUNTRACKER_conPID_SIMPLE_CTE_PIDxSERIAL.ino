...
// PUNTO MUERTO DEL SISTEMA (SOLO PARA C. PROPORCIONAL)
#define ERROR_MINIMO_X 0  //10
#define ERROR_MINIMO_Z 0  //10

#define TOLERANCIA_ERROR 1023

// VARS DE LECTURA DE LOS SENSORES LDR
int valor_ldr_W = 0, valor_ldr_S = 0, valor_ldr_N = 0, valor_ldr_E = 0;

// CONSTANTES CONTROLADOR PID
float Kp = 24;   //Ajuste de ganancia proporcional ()
float Ki = 0.001;  //Ajuste de ganancia integral     ()
float Kd = 0.1;   //Ajuste de ganancia derivativa   ()

// CALCULO DE dt
float tiempo_anterior = 0;

// ERRORES DEL CONTROLADOR
float error_horizontal_total = 0, error_vertical_total = 0;
...
// ------------------- LOOP -------------------
void loop() {
  // Comentamos por si SOLO queremos realizar la grafica de los errores.
  leerEntradaSerial();
  ...
  ajustarVelocidad(error_horizontal_total, &tiempo_pulso_vertical);
  ajustarVelocidad(error_vertical_total, &tiempo_pulso_horizontal);
...
void orientarSeguidor() {
  ...
  if (error_horizontal > 0) {
    ...
    moverHorizontal(-pasos_horizontal);
...
int leerSuavizado(int pin) {
    ...
    for (int i = 0; i < NUM_LECTURAS; i++) {
        suma += analogRead(pin);
        delayMicroseconds(5); // Pequeña espera entre lecturas
    }
...
void calcularErrores() {
  ...
  // Solo para controlador P
  if (abs(error_horizontal) < ERROR_MINIMO_X && Ki == 0 && Kd == 0) 
  {
    //Serial.print("\n----- PUNTO MUERTO X -----");
    error_horizontal = 0;
  ...
  if (abs(error_vertical) < ERROR_MINIMO_Z && Ki == 0 && Kd == 0) 
  {
    //Serial.print("\n----- PUNTO MUERTO Z -----");
    error_vertical = 0;
...
void ajustarVelocidad(int error, int *tiempo_pulso) {
  //Serial.print("Error recibido: ");
  //Serial.println(error);

  *tiempo_pulso = map(abs(error), 0, TOLERANCIA_ERROR, T_VEL_MAX, T_VEL_MIN);
  *tiempo_pulso = constrain(*tiempo_pulso, T_VEL_MIN, T_VEL_MAX);

  //Serial.print("Nuevo tiempo de pulso: ");
  //Serial.println(*tiempo_pulso);
}


// Transformamos el error prop, int y derivativamente
void controladorPID() {
  ...
  error_horizontal_total = Kp * error_horizontal + Ki * integral_error_horizontal + Kd * derivada_error_horizontal;
  error_vertical_total = Kp * error_vertical + Ki * integral_error_vertical + Kd * derivada_error_vertical;

  pasos_horizontal = error_horizontal_total;
  pasos_vertical = error_vertical_total;
...
void moverVertical(int pasos) {
  ...
  } else digitalWrite(PAP_DIR_Z, HIGH);
...
void mostrarPasos() {
  Serial.println("\n== Pasos ==");
  Serial.print("Pasos Horizontal: ");
  Serial.print(pasos_horizontal);
  Serial.print("  | Pasos Vertical: ");
  Serial.println(pasos_vertical);
}
